...
function parseJsonBody(req) {
  return new Promise((resolve, reject) => {
    // If body already parsed (e.g. Netlify function or express middleware)
...
function requireStudentAuth(req, url) {
  const token = getBearerToken(req, url);
  if (!token) return null;
  const session = db.getSession(token);
  if (!session || !session.student_id) return null;
  const student = db.getStudentById(session.student_id);
...
export async function handleApiRequest(req, res) {
  ...
  if ((pathname === '/api/auth/join' || pathname === '/api/auth/register') && method === 'POST') {
    try {
      ...
      const token = db.createSession(student.userId, student.studentId, 'student');
  ...
  if (pathname === '/api/auth/login' && method === 'POST') {
    try {
      ...
      if (body.role === 'teacher' || (!body.email && body.password)) {
        const valid = db.verifyTeacherPassword(body.password);
        if (!valid) return sendError(res, 401, 'Incorrect teacher password. Please try again.');
        const user = db.db.prepare(`SELECT user_id FROM users WHERE role = 'teacher'`).get();
        const token = db.createSession(user.user_id, null, 'teacher', 24);
        const settings = db.getClassSettings();
        return sendJson(res, 200, {
          success: true,
          token,
      ...
      const student = db.verifyStudentCredentials(email, password);
      if (!student) {
  ...
  if (pathname === '/api/student/activities' && method === 'GET') {
    const auth = requireStudentAuth(req, url);
    if (!auth) return sendError(res, 401, 'Unauthorized');
    const attempts = db.db.prepare(`SELECT * FROM activity_attempts WHERE student_id = ? ORDER BY completed_at DESC`).all(auth.student.id);
    return sendJson(res, 200, { success: true, activities: attempts });
  }
  ...
  // ------------------------------------------------------------------------
  // 5. SERVER-SIDE XP & SUBMISSIONS (SECTIONS 14, 15, 19, 20, 23)
  // ------------------------------------------------------------------------
  ...
  if (learnMatch && method === 'POST') {
    ...
    const result = db.recordTopicLearn(auth.student.id, topicId);
  ...
  if (practiceMatch && method === 'POST') {
    ...
    const result = db.recordTopicPractice(auth.student.id, topicId, count);
    broadcastSSE('leaderboard_update', { studentId: auth.student.id, topicId });
    return sendJson(res, 200, { success: true, result });
  }
  ...
  if (isQuizSubmit) {
    ...
    const result = db.recordQuizSubmission({
  ...
  if ((pathname === '/api/activities/complete' || pathname === '/api/activities/submit') && method === 'POST') {
    ...
    const result = db.recordActivityCompletion({
      studentId: auth.student.id,
      topicId,
      activityType,
  ...
  if (rpCompleteMatch && method === 'POST') {
    ...
    const result = db.recordRoleplayAttempt({
      studentId: auth.student.id,
      roleplayId,
      score,
      speakingScore,
  ...
  // ------------------------------------------------------------------------
  // 6. PUBLIC QUESTIONS & RANDOM PRACTICE (SECTION 28)
  // ------------------------------------------------------------------------
  ...
  if (qByTopicMatch && method === 'GET') {
    ...
    const questions = db.getQuestionsByTopic(topicId, limit, randomize);
    return sendJson(res, 200, { success: true, questions });
  }

  // ------------------------------------------------------------------------
  // 7. REAL LEADERBOARD (SECTION 25: NO EMAIL PRIVACY LEAK)
  // ------------------------------------------------------------------------
  ...
  if (pathname === '/api/curriculum' && method === 'GET') {
    return sendJson(res, 200, { success: true, topics: db.getCurriculumTopics(false) });
  }

  if (pathname === '/api/roleplays' && method === 'GET') {
    return sendJson(res, 200, { success: true, roleplays: db.getRoleplays(false) });
  }

  // ------------------------------------------------------------------------
  // 9. TEACHER / ADMIN DASHBOARD & MANAGEMENT (SECTIONS 9, 31, 35)
  // ------------------------------------------------------------------------
  if (pathname === '/api/admin/login' && method === 'POST') {
    try {
      ...
      const valid = db.verifyTeacherPassword(password);
      if (!valid) return sendError(res, 401, 'Incorrect teacher password. Please try again.');
  ...
  if (pathname === '/api/admin/me' && method === 'GET') {
    const admin = requireAdminAuth(req, url);
  ...
  if (studentDetailMatch && method === 'GET') {
    ...
    const profile = db.getStudentDetailedProfile(studentId);
  ...
  if (studentDetailMatch && method === 'DELETE') {
    ...
    const studentId = studentDetailMatch[1];
    const success = db.deleteStudentById(studentId);
  ...
  // Admin: Questions Management (GET/POST/PATCH/DELETE /api/admin/questions)
  if (pathname === '/api/admin/questions' && method === 'GET') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    const topicId = url.searchParams.get('topicId') || null;
    const questions = db.getAllQuestionsAdmin(topicId);
  ...
  if (pathname === '/api/admin/questions' && method === 'POST') {
    ...
    try {
      const body = await parseJsonBody(req);
      const qId = db.createQuestion(body);
  ...
  if (qAdminMatch && method === 'PATCH') {
    ...
    try {
      const body = await parseJsonBody(req);
      db.updateQuestion(qAdminMatch[1], body);
  ...
  if (qIdToDelete && method === 'DELETE') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    db.deleteQuestion(qIdToDelete);
    return sendJson(res, 200, { success: true, deleted: qIdToDelete });
  }

  // Admin: Real-Time Notifications (GET /api/admin/notifications)
  if (pathname === '/api/admin/notifications' && method === 'GET') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    const limit = parseInt(url.searchParams.get('limit') || '50', 10);
    const notifications = db.getNotifications('teacher', limit);
  ...
  if ((pathname === '/api/admin/notifications/read' || pathname === '/api/admin/notifications/mark-read') && (method === 'PATCH' || method === 'POST')) {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    db.markNotificationsAsRead('teacher');
    return sendJson(res, 200, { success: true, message: 'All notifications marked as read' });
  }

  // Admin: Curriculum Management (GET, POST, PATCH, DELETE)
  if (pathname === '/api/admin/curriculum' && method === 'GET') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    return sendJson(res, 200, { success: true, topics: db.getCurriculumTopics(true) });
  }
  ...
  if (currToggleMatch && method === 'PATCH') {
    ...
    if (!admin) return sendError(res, 401, 'Unauthorized');
    try {
      const active = db.toggleCurriculumTopic(currToggleMatch[1]);
  ...
  if (currDeleteMatch && method === 'DELETE') {
    ...
    if (!admin) return sendError(res, 401, 'Unauthorized');
    try {
      db.deleteCurriculumTopic(currDeleteMatch[1]);
  ...
  if (pathname === '/api/admin/curriculum/reset' && method === 'POST') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    const topics = db.resetCurriculumTopics();
    broadcastSSE('curriculum_updated', { reset: true });
    return sendJson(res, 200, { success: true, topics });
  }

  // Admin: Roleplay Management (GET, POST, PATCH, DELETE)
  if (pathname === '/api/admin/roleplays' && method === 'GET') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    return sendJson(res, 200, { success: true, roleplays: db.getRoleplays(true) });
  }
  ...
  if (rpToggleMatch && method === 'PATCH') {
    ...
    if (!admin) return sendError(res, 401, 'Unauthorized');
    try {
      const active = db.toggleRoleplayActive(rpToggleMatch[1]);
  ...
  if (rpUpdateMatch && method === 'PUT') {
    ...
    try {
      const body = await parseJsonBody(req);
      const updated = db.updateRoleplay(rpUpdateMatch[1], body);
  ...
  if (rpDeleteMatch && method === 'DELETE') {
    ...
    if (!admin) return sendError(res, 401, 'Unauthorized');
    try {
      db.deleteRoleplay(rpDeleteMatch[1]);
  ...
  if (pathname === '/api/admin/roleplays/reset' && method === 'POST') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    const roleplays = db.resetRoleplays();
  ...
  if (pathname === '/api/admin/settings' && method === 'GET') {
    const admin = requireAdminAuth(req, url);
    if (!admin) return sendError(res, 401, 'Unauthorized');
    return sendJson(res, 200, { success: true, settings: db.getClassSettings() });
  }
  ...
  if (pathname === '/api/admin/settings' && method === 'PUT') {
    ...
    try {
      const body = await parseJsonBody(req);
      const updated = db.updateClassSettings(body);
  ...
  if (pathname === '/api/admin/change-password' && method === 'POST') {
    ...
    try {
      ...
      if (currentPassword && !db.verifyTeacherPassword(currentPassword)) {
      ...
      db.updateTeacherPassword(newPassword);
